...
fun StudentResourcesScreen() {
    ...
    Column(
        ...
        Text("Aquí podrás consultar las materias vinculadas por código de clase y guardar recursos personales sin modificar información enviada por el docente.")

        ElevatedCard(Modifier.fillMaxWidth()) {
            Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    Icon(Icons.Default.Description, contentDescription = null)
                    Text("Documentos de estudio", style = MaterialTheme.typography.titleMedium)
                }
                ...
                Button(onClick = { picker.launch(supportedDocumentTypes) }) {
                    ...
                    Text(if (uriString == null) "Importar recurso" else "Cambiar recurso")
                }
                ...
                if (uriString != null) {
                    OutlinedButton(onClick = {
                        ...
                        runCatching { context.startActivity(intent) }
                            .onFailure { message = "No hay una aplicación compatible para abrir este archivo." }
                    }) {
        ...
        Text("Los archivos guardados aquí son recursos locales del estudiante y no cambian calificaciones, asistencia ni datos del docente.", style = MaterialTheme.typography.bodySmall)
    }
}
